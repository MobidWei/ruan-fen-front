      ...
      <div class="content">
          <div class="doc">
            ...
            <div class="author-info">
              ...
              <h3>
                <span>
                  <router-link style="color: #2E86C1;" to="">{{ author.institution }}</router-link>
                </span>
              </h3>
              <h3>
                <span>
            ...
            <div id="author-article">
              <div class="doc-title">
                <h2>作者文献</h2>
                <h3>
                  <router-link to="">认领门户</router-link>后可管理自己的文献信息
                </h3>
              </div>
              <div class="doc-content">
                <ul>
                  <li v-for="(project, index) in projects" :key="project.id">
                    [{{ index + 1 }}]  <router-link>{{ project.name }}</router-link>  {{ project.author.join(',') }}
                  </li>
                </ul>
              </div>
            </div>
            <div id="author-patent">
              <div class="doc-title">
                <h2>作者专利</h2>
                <h3>
                  <router-link to="">认领门户</router-link>后可管理自己的专利信息
                </h3>
              </div>
              <div class="doc-content">
                <li v-for="(patent, index) in patents" :key="patent.id">
                  [{{ index + 1 }}]  <router-link>{{ patent.name }}</router-link>  {{ patent.author.join(',') }}
                </li>
              </div>
            </div>
            ...
            <div id="author-prize">
              ...
              <div class="doc-content">
                <li v-for="(prize, index) in prizes" :key="prize.id">
                  [{{ index + 1 }}]  <router-link>{{ prize.name }}</router-link>  {{ prize.level }}
                </li>
              </div>
            </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const duplicated_authors = ref([
      { id: 0 , name: "帅哥", institution:"北京医科大学第三附属医院", domain:"心血管疾病" },
    ...
    const patents = ref([
      { id: 0, name: "棒料锯切自动在线光电控制等重下料的计算机控制方法", author: [ "李先禄", "吴键", "张志明", "沈蕾芳"] }
    ]);
    const projects = ref([
      { id: 0, name: "多模无线通信光纤分布设备研发及应用", author: ["邓泽林", "杨建明", "孙玉静", "吕昕范辉"] }
    ]);
    const prices = ref([
      { id: 0, name: "国家科技成果", level: "二等奖" }
    ]);
    const author = ref({
      name: "帅哥", 
    ...
    return { name, duplicated_authors, author, articles, patents, projects, prices };
  },
});
</script>
...
#duplicated-author li, #duplicated-author li a {
  font-size: 14px;
  /* font-weight: bold; */
  color: black;
}
...
#author-article, #author-patent, #author-project, #author-prize, #author-cooperation{
  margin-top: 15px;
  clear: both;
  min-height: 200px;
