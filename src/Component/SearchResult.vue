    ...
    <div class="table__con">
      <div class="table__search flex align-center justify-between">
        ...
        <div class="table__search__fr flex align-center">
          ...
          <div class="page flex align-center justfiy-center">
            <span class="btn-next flex align-center justify-center pointer"
      ...
      <div class="filter__se flex align-center justify-between">
        <div class="filter__se__lf flex align-center">
          ...
          />
          <div class="m-l-20">
            已选<span class="col1 m-l-5">{{ multipleSelection.length }}</span>
          </div>
        ...
        <div class="filter__se__rf flex align-center">
          <span
            >排序：<span
              class="pointer"
              :class="{ active: searchParams.relevant !== '' }"
              @click="handleFilter('relevant')"
              >相关度<el-icon v-if="searchParams.relevant === 1"
                ><Bottom /></el-icon
              ><el-icon v-if="searchParams.relevant === 2"
                ><Top /></el-icon></span
          ></span>
          <span
            class="m-l-20 pointer"
            :class="{ active: searchParams.publishTime !== '' }"
            @click="handleFilter('publishTime')"
            >发表时间<el-icon v-if="searchParams.publishTime === 1"
              ><Bottom /></el-icon
            ><el-icon v-if="searchParams.publishTime === 2"><Top /></el-icon
          ></span>
          <span
            class="m-l-20 pointer"
            :class="{ active: searchParams.reference !== '' }"
            @click="handleFilter('reference')"
            >被引<el-icon v-if="searchParams.reference === 1"
              ><Bottom /></el-icon
            ><el-icon v-if="searchParams.reference === 2"><Top /></el-icon
          ></span>
          <span
            class="m-l-20 pointer"
            :class="{ active: searchParams.look !== '' }"
            @click="handleFilter('look')"
            >浏览<el-icon v-if="searchParams.look === 1"><Bottom /></el-icon
            ><el-icon v-if="searchParams.look === 2"><Top /></el-icon
          ></span>
          <span
            class="m-l-20 pointer"
            :class="{ active: searchParams.type !== '' }"
            @click="handleFilter('type')"
            >综合<el-icon v-if="searchParams.type === 1"><Bottom /></el-icon
            ><el-icon v-if="searchParams.type === 2"><Top /></el-icon
          ></span>
          <div class="flex align-center m-l-20">
            <span class="flex-shrink">显示：</span>
            <el-select v-model="searchParams.pageSize" style="width: 60px">
              <el-option
                v-for="item in pages"
                :key="item.value"
                :label="item.label"
          ...
          <span class="m-l-20 btn-sel pointer"
            ><el-icon><FullScreen /></el-icon
          ></span>
          <span class="m-l-10 btn-sel pointer"
      ...
      <div class="table__cont__info">
        ...
        >
          ...
          <el-table-column prop="author" label="作者" width="160">
            <template #default="scope">
              <router-link
                :to="{ path: '/ResearchUser', query: { id: scope.row.id, name: scope.row.author } }"
...
const handleFilter = (tag: keyof Params) => {
  ...
  searchParams.value[tag] =
    searchParams.value[tag] === "" ? 1 : searchParams.value[tag] === 1 ? 2 : 1;
};
</script>
