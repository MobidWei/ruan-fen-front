    ...
    <div class="full-width">
        ...
        <span class="sele flex flex-shrink justify-center align-center pointer">
          {{ currentField }}<el-icon class="m-l-5"><CaretBottom/></el-icon>
                    ...
                    <template #default>
                        <ul class="pop__menu">
                            <li class="pointer" @click="selectTheme('专利名')">专利名</li>
                            <li class="pointer" @click="selectTheme('申请人')">申请人</li>
                            <li class="pointer" @click="selectTheme('发明人')">发明人</li>
                ...
                <div class="pointer flex-shrink ques flex align-center justify-center">
                    ...
                    >
                        <template #reference>
        <span class="sele flex flex-shrink justify-center align-center pointer">
          {{ type }}<el-icon class="m-l-5"><CaretBottom/></el-icon>
        </span>
                        </template>
                        <template #default>
                            <ul class="pop__menu">
                                <li>
                                    <router-link class="pointer" @click="selectType('文献')"
                                                 :to="{ path: '/search/article', query: { field:'全部',question: props.question} }">
                                        文献
                                    </router-link>
                                </li>
                                <li>
                                    <router-link class="pointer" @click="selectType('专利')"
                                                 :to="{ path: '/search/patent', query: { field:'全部',question: props.question } }">
                                        专利
                                    </router-link>
                                </li>
                            </ul>
...
const currentField = ref('全部');
const type = ref('专利');
const selectTheme = (value: string) => {
    currentField.value = value;
    if (value === '申请人') {
        searchParams.value.field = 'applicants'
    } else if (value === '发明人') {
        searchParams.value.field = 'inventorsName'
    } else if (value === '研究领域') {
        searchParams.value.field = 'fieldOfResearch'
    } else if (value === '专利名') {
        searchParams.value.field = 'patentName'
    } else {
