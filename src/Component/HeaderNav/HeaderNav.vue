<template>
        ...
        <el-row>
          ...
          <el-col :span="11">
            <div class="header__conta__menu flex align-center justify-center">
              <div class="total__menu"><span>总库</span><el-icon><CaretBottom /></el-icon></div>
              <ul class="flex align-center menu__lis">
                <li class="flex align-center justify-center pointer" v-for="item in menus" :key="item.id" :class="{'current':item.id === currentId}" @click="handleMenu(item)"><span>{{ item.label }}</span><el-icon><ArrowDown /></el-icon></li>
          ...
          <el-col :span="8">
            <div class="right__menu flex align-center">
              ...
              <span class="p__login pointer">个人登录</span>
          </div>
          </el-col>
        </el-row>
      </div>
  ...
  >
    <template #reference>
      ...
      <template #default>
        <ul class="pop__menu">
          <li class="pointer">主题2</li>
          <li class="pointer">主题3</li>
          <li class="pointer">主题4</li>
        </ul>
        ...
        <div class="pointer flex-shrink ques flex align-center justify-center"><el-icon><ChatLineRound /></el-icon><span>问答</span></div>
      </div>
      ...
      <div class="submenu__con flex align-center">
        <div class="total__sub flex align-center">
          ...
          <div class="lan flex align-center justify-center flex-column">
            ...
            <span v-for="item in langs" :key="item.value" class="flex align-center justify-center" :class="{'current':currentLang === item.value}" @click="changeLang(item)">{{item.label}}</span>
          </div>
        </div>
        <div class="all__sub flex-1">
          <div class="all__sub__inner  flex align-center flex-1">
          <div v-for="item in submenu" :key="item.id" class="flex flex-column align-center  flex-1 pointer submenu__item ">
            <span class="b_b">{{ item.title }}</span>
            <span class="s_b">{{ item.count }}</span>
            <el-icon v-show="item.isArrow"><CaretBottom /></el-icon>
